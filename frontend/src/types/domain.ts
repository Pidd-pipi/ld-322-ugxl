...
export interface Alert { id: number; greenhouseId: number; sensorId: number; level: string; message: string; value: number; status: string; createdAt: string; handledAt?: string; handledBy?: string; handleNote?: string; sensor?: Sensor; }
export interface Device { id: number; greenhouseId: number; name: string; type: string; status: 'on' | 'off'; updatedAt: string; }
export interface EnvironmentReport { greenhouseId: number; range: string; generatedAt: string; alerts: number; pendingAlerts: number; handledAlerts: number; metrics: Record<string, { average: number; min: number; max: number; unit: string }> }
